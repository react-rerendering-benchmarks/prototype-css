...
export const Popup = memo(() => {
  ...
  const handleSwitch = async (cssProto: CSSProto) => {
    ...
    if (cssProto.isActive) removeCss(cssProto);else injectCss(cssProto);
    updateProtos();
  };
  return <>
      <div style={{
      padding: '1rem 0'
        ...
        <div className="my-2">
          {cssProtos.map(proto => <div style={{
          display: 'flex'
        }}>
              <label className="ellipsis" style={{
            marginRight: 'auto'
          }}>
        ...
        <a href={`options.html#${short.generate()}=//${url?.hostname}`} role="button" className="center-center outline" style={{
        fontSize: '1.5rem',
        padding: '.2rem',
        lineHeight: 1
      }} target="_blank">
